...
namespace studeehub.Application.Services
{
	public class PomodoroSessionService : IPomodoroSessionService
	{
		...
		public async Task<BaseResponse<GetSessionResponse>> StartSessionAsync(Guid userId, PomodoroType type)
		{
			...
			if (active != null)
				return BaseResponse<GetSessionResponse>.Fail("You already have an active Pomodoro session.", ErrorType.Validation);
			...
			var response = _mapper.Map<GetSessionResponse>(newSession);
			return BaseResponse<GetSessionResponse>.Ok(response, "Pomodoro session started.");
		}
		...
		public async Task<BaseResponse<GetSessionResponse>> SkipSessionAsync(Guid userId)
		{
			...
			var settings = await _pomodoroSettingService.GetForUserAsync(userId);
			if (!activeSession.IsAutoGenerated)
				return BaseResponse<GetSessionResponse>.Ok(_mapper.Map<GetSessionResponse>(activeSession), "Session skipped.");
