...
{
	public class UpdateAchievemValidator : AbstractValidator<UpdateAchievemRequest>
	{
		public UpdateAchievemValidator()
		{
			RuleFor(x => x.Code).NotEmpty().WithMessage("Code is required.");
			RuleFor(x => x.Name).NotEmpty().WithMessage("Name is required.");
			// Description is nullable on DTO; do not force NotEmpty here.
			...
			RuleFor(x => x.RewardValue).GreaterThanOrEqualTo(0).WithMessage("RewardValue must be non-negative.");
		}
	}
}
